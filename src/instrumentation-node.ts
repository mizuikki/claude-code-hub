...
export async function register() {
  ...
  // 开发环境: 执行迁移 + 初始化价格表（禁用 Bull Queue 避免 Turbopack 冲突）
    ...
    if (isConnected) {
      ...
      logger.warn("Database not available in development, skipping migrations and price init");
    }

    // 同步错误规则并初始化检测器（开发环境也需要）
    try {
      await syncErrorRulesAndInitializeDetector();
    } catch (error) {
      logger.error("[Instrumentation] Failed to initialize error rule detector in development", {
        error: error instanceof Error ? error.message : String(error),
      });
      // 开发环境继续运行，便于调试数据库连接问题
    }
